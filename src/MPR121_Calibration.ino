...
void printTouchData (String label) {
  Serial.print(label + " " + char(addr[i] - 25)+ String(j) + ", "); //addr[i]-25 = A,B,C,D in ASCII
  Serial.print("Base:" + String(baselineData(i,j)));
  Serial.print(" Filt:" + String(filteredData(i,j)));
  Serial.println(" diff:" + String(baselineData(i,j) - filteredData(i,j)));
}

void runOutput() {
  for (i=0;i<n;i++) {  //read touch, filtered & baseline data @ same time stamp of breakout boards 
    Wire.requestFrom(addr[i],42);  //registers from 00x00 to 00x2A
    j=0;
  ...
  if (oType == 0) {//touched-released status of electrode[j] in board[i]
    ...
    for (j=0;j<12;j++) {
      ...
      Serial.print(filteredData(oType - 7,j)); Serial.print(","); }
    Serial.println(" ");
  }
